...
function MergeChr(a, b, ai, bi)
{
  ...
  while(i <= a.chrs[ai].end && j <= b.chrs[bi].end)
  {
    ...
    if(b.values[j].pos < a.values[i].pos)
    {
      ...
      for(var k = 0; k < a.ncov; k++){ b.values[j].cover.splice(0, 0, 0); }
    ...
    else if(a.values[i].pos < b.values[j].pos)
    {
      ...
      for(var k = 0; k < b.ncov; k++){ a.values[i].cover.push(0); }
    ...
    {
      //Add the b values
      for(var k = 0; k < b.ncov; k++){ a.values[i].cover.push(b.values[j].cover[k]); }
...
function CompleteChr(a, i, n, p)
{
  ...
  for(var j = a.chrs[i].start; j <= a.chrs[i].end; j++)
  {
    ...
    for(var k = 0; k < n; k++)
    {
      ...
      {
        //Insert at the start
        a.values[j].cover.splice(0, 0, 0);
      }
      else
      {
        //Insert at the end
        a.values[j].cover.push(0);
      }
    }
